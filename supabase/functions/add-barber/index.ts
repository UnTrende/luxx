...
serve(async (req) => {
  ...
  try {
    ...
    console.log("add-barber function called", undefined, 'index');
    ...
    console.log("User authentication result:", { user: user?.id, error: userError?.message }, 'index');
    ...
    console.log("Request body:", JSON.stringify(requestBody, null, 2, 'index'));
    ...
    if (existingUserError) {
      console.error("Error checking existing user:", existingUserError, 'index');
    }
    ...
    if (updateError) {
      console.error("Error updating app_users record:", updateError, 'index');
    ...
    console.log('Barber profile creation result', { 
      ...
      barberId: profileResult.data?.id 
    }, 'add-barber');
    ...
    console.error("Error in add-barber function:", error, 'index');
